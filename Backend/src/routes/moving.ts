...
import { importMovingItems, listMovingItems, normalizeCandidates } from '../services/moving';

const router = Router();
// Conversational import moved to MCP's prepare_import / commit_changes.
// Keep historical batches and ordinary item listing/manual entry accessible.
router.post(['/import','/import/preview'], (_req,res) => {
  res.status(410).json({error:'站内对话导入已移除。请通过 MCP prepare_import / commit_changes 录入，或使用物品管理手动添加。'});
});
...
router.post('/items', (req, res, next) => {
  try {
    const candidates = normalizeCandidates([req.body]);
    const result = importMovingItems(candidates, JSON.stringify(candidates), 'manual');
