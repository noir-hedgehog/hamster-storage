...
export default function StoragePreview({onClose}: {onClose:()=>void}) {
  const {locations,rooms,storages,items,getStoragePath,refreshData}=useStorage();
  const [furniture,setFurniture]=useState<Furniture[]>([]);
  const [refreshError,setRefreshError]=useState('');
  async function refresh() {
    try {const [data]=await Promise.all([apiClient.getFurniture(),refreshData(true)]);setFurniture(data);setRefreshError('');}
    catch {setRefreshError('刷新失败，当前显示上次读取的内容。请重试。');}
  }
  ...
  const visibleFurniture=useMemo(()=>furniture.filter(f=>visibleRooms.some(r=>r.id===f.roomId)),[furniture,visibleRooms]);
  ...
  const selectedFurniture=visibleFurniture.find(f=>f.id===selected);
  const furnitureItems=selectedFurniture?.storageId?related.get(selectedFurniture.storageId)||[]:[];
  ...
  const inferred=visibleRooms.some(r=>!r.geometry&&[r.floorplanX,r.floorplanY,r.floorplanWidth,r.floorplanHeight].some(v=>!Number.isFinite(v))) || visibleStorages.filter(s=>!s.parentStorageId&&!visibleFurniture.some(f=>f.storageId===s.id)).some(s=>[s.floorplanX,s.floorplanY,s.floorplanWidth,s.floorplanHeight].some(v=>!Number.isFinite(v)));
  ...
  return <section className="space-page"><header className="space-header"><div><p className="space-eyebrow">SPACE EXPLORER</p><h2>让收纳，一目了然。</h2><p>沿着房间找到收纳，再找到你要的那件东西。</p></div>
    <div className="space-toolbar"><button onClick={()=>void refresh()}>刷新 Agent 修改</button><button onClick={()=>setEditor(true)}><MapIcon size={16}/>旧示意布局编辑</button><button onClick={onClose}><ArrowLeft size={16}/>返回收纳</button></div></header>
    ...
    <div className="space-filters"><label>地点<select aria-label="地点" value={home} onChange={e=>{setHome(e.target.value);setRoom('');setSelected('');}}><option value="">全部地点</option>{locations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label>
      ...
      <span className="space-summary">{visibleRooms.length} 个房间 · {visibleFurniture.length} 件家具 · {visibleStorages.length} 个收纳 · {total.length} 种物品</span></div>
    <div className="space-workspace"><aside className="space-library"><label className="space-search"><Search size={17}/><input aria-label="搜索收纳或物品" value={query} onChange={e=>setQuery(e.target.value)} placeholder="找收纳或物品…"/></label>
      {!!visibleFurniture.length&&<><h3>实测家具 <span>{visibleFurniture.length}</span></h3><div className="space-storage-list">{visibleFurniture.filter(f=>!query||f.name.includes(query)).map(f=><button key={f.id} aria-pressed={selected===f.id} onClick={()=>setSelected(f.id)}><Box size={18}/><span><strong>{f.name}</strong><small>{f.width} × {f.depth} × {f.height} cm</small></span></button>)}</div></>}
      ...
      <div className="space-stage"><div className="space-stage-tools"><div className="space-segment"><button aria-pressed={view==='perspective'} onClick={()=>setView('perspective')}>立体</button><button aria-pressed={view==='top'} onClick={()=>setView('top')}>俯视</button></div><button onClick={()=>setReset(n=>n+1)}><RotateCcw size={16}/>视角复位</button></div>
        {visibleRooms.length?<Scene rooms={visibleRooms} storages={visibleStorages} furniture={visibleFurniture} selected={selected} onSelect={setSelected} view={view} reset={reset}/>:<div className="space-empty-stage"><Layers size={40}/><h3>给物品一个家</h3><p>添加房间和收纳位置后，就能在这里查看。</p></div>}
        <div className="space-stage-note"><span>拖动旋转 · 滚轮缩放 · 双指缩放 / 平移</span><span>{visibleRooms.some(r=>r.geometry)?'实测房间优先；家具按录入尺寸显示':'旧布局为示意，未经比例校准'}{inferred?' · 含自动排列':''} · 墙体及未关联家具的收纳高度为示意</span></div></div>
      <aside className="space-detail">{selectedFurniture?<><p className="space-eyebrow">已选家具 · cm</p><h3>{selectedFurniture.name}</h3><p className="space-path">{rooms.find(r=>r.id===selectedFurniture.roomId)?.name}</p><p>宽 {selectedFurniture.width} · 深 {selectedFurniture.depth} · 高 {selectedFurniture.height}</p><p>中心位置 {selectedFurniture.x}, {selectedFurniture.y} · 旋转 {selectedFurniture.rotation}°</p><p className="space-path">{selectedFurniture.description||'尺寸来自录入数据，尚未代表现场摆放验证。'}</p>{selectedFurniture.storageId?<><h4>关联收纳内容（含下级）</h4>{furnitureItems.map(item=><article key={item.id}><b>{item.name}</b><span>{item.quantity} {item.unit||'件'}</span></article>)}</>:<p className="space-empty">未关联收纳容器。</p>}</>:selection?<><p className="space-eyebrow">已选收纳</p><h3>{selection.name}</h3><p className="space-path">{getStoragePath(selection.id)}</p><div className="space-detail-count"><strong>{selectionItems.length}</strong><span>种物品{!selection.parentStorageId?'（含下级收纳）':''}</span></div>
