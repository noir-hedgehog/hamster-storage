...
- 后端构建通过，39 项测试通过；前端构建、相关文件类型检查及 11 项测试通过。
...
- 数据库驱动固定为 `better-sqlite3@12.10.1`，Docker 构建与运行均使用 Node 24，与本地验证环境对齐；未使用生产库进行写入测试。
- utopia 发布验收单独记录在 `ops/utopia/MCP-RELEASE.md`，不能用本地验证代替线上验证。本文的连接模板不代表已经替外部 agent 写入配置；Sites 不在本次更新范围内。
